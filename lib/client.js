...
/**
 * Live-state memory, kept per host between polls.
 *
 * Why this exists rather than one more field on the wire: the peer reports
 * `ageMs` as the age of the session's last HUMAN PROMPT
 * (`SessionSummary.updatedAt` is `max(createdAt, lastPromptAt)`), and nothing
 * in that number moves when a run finishes. So "it completed while you were
 * away" cannot be read off any single snapshot — a session prompted 7 minutes
 * ago and still running reports the same shape as one that finished a second
 * ago. Only the running→idle EDGE says it, and an edge needs two observations.
 */
var liveBaseline = new Map();

/**
 * Sessions that finished a run since each host's view was last opened.
 *
 * Cleared by {@link createStatusStore}'s `setViewing`, which is the same moment
 * the green dot is supposed to go out.
 */
var pendingCompletions = new Map();
...
function createStatusStore() {
  ...
  return {
     ...
     * waiting for the next poll — including the completions observed since the
     * last visit, which are the ones no snapshot can carry.
    ...
    setViewing: function (hostId) {
      ...
      pendingCompletions.delete(hostId);
      var changed = false;
 ...
 * @param now - the poll's clock, shared with {@link accountCompletions}.
 * @param viewing - whether this host's remote view is open right now.
 * @returns a per-host aggregate row; never rejects.
 */
function readPeer(host, elapsedMs, now, viewing) {
  var base = { id: host.id, name: host.name, url: host.url };
  // A host we cannot read has no observable history: drop the baseline so a
  // peer that restarts (or a tab that was asleep) re-baselines instead of
  // reporting every session as having just finished.
  var lost = function (row) {
    liveBaseline.delete(host.id);
  ...
  return fetch(host.url + PEER_STATUS_PATH, {
    ...
    if (!response.ok) return lost({ reachable: true, peer: false });
    return response.json().then(function (body) {
      if (body === null || typeof body !== "object" || PEER_STATUS_VERSIONS.indexOf(body.version) === -1) {
        return lost({ reachable: true, peer: false });
      }
      if (body.available !== true) {
        return lost({ reachable: true, peer: true, waiting: 0, running: 0, unread: 0 });
      }
      ...
      sessions.forEach(function (session) {
        ...
        // A session blocked on the operator is still a live one, so it feeds the
        // baseline: running→waiting must not read as a completion.
        if (session.running === true) live += 1;
        ...
        if (isUnreadPrompt(session, elapsedMs)) unread += 1;
      });
      accountCompletions(host.id, live, viewing);
      unread += pendingCompletions.get(host.id) || 0;
      // Both signals can describe one episode (prompted from the remote's own
      // GUI, then finished), so a host can never report more unread sessions
      // than it has sessions.
      if (unread > sessions.length) unread = sessions.length;
...
/**
 * Arm a completion reminder when a host's live sessions stop.
 *
 * This is the running→idle edge, the same fact the remote's own sidebar draws
 * as its green "done" dot. It cannot come from one snapshot — see
 * {@link liveBaseline} — so it is tracked here, between polls.
 *
 * While the view is open nothing is armed: the operator is watching, so there
 * is nothing to remind them of, and the baseline is still advanced so that a run
 * ending after they leave is caught.
 * @param hostId - registry row id.
 * @param live - sessions the peer reports as running right now.
 * @param viewing - whether this host's remote view is open right now.
 */
function accountCompletions(hostId, live, viewing) {
  var previous = liveBaseline.get(hostId);
  liveBaseline.set(hostId, live);
  if (viewing === true || previous === undefined) return;
  var finished = previous - live;
  if (finished <= 0) return;
  pendingCompletions.set(hostId, (pendingCompletions.get(hostId) || 0) + finished);
}

/**
 * Whether one peer session carries a prompt newer than this browser's last look.
 *
 * `session.ageMs` is how long ago the peer last received a HUMAN prompt — not
 * how long ago it was touched, which is why a completion is invisible here and
 * is tracked as an edge by {@link accountCompletions} instead. `elapsedMs` is
 * how long ago this browser last looked, so both are durations and the verdict
 * never depends on the two machines' clocks agreeing. A running session is never
 * "unread": it is reported by the running count instead.
 * @param session - one `{ running, ageMs }` row from a peer.
 * @param elapsedMs - milliseconds since this browser last viewed the host.
 * @returns true when the prompt arrived after the last look.
 */
function isUnreadPrompt(session, elapsedMs) {
  if (session === null || typeof session !== "object") return false;
  ...
  var run = function () {
    api("/hosts", { method: "GET" }).then(function (body) {
      ...
      return Promise.all(hosts.map(function (host) {
        ...
        return readPeer(host, viewing ? 0 : Math.max(0, now - readLastSeen(host.id)), now, viewing);
      }));
    }).then(function (rows) {
      if (stopped) return;
      // Forget hosts that left the registry, so a removed-and-re-added host
      // re-baselines instead of comparing against a stale count.
      var present = new Set(rows.map(function (row) { return row.id }));
      liveBaseline.forEach(function (value, hostId) { if (!present.has(hostId)) liveBaseline.delete(hostId) });
      pendingCompletions.forEach(function (value, hostId) {
        if (!present.has(hostId)) pendingCompletions.delete(hostId);
      });
 ...
 * own session dots — including the animated chase for "running" — so the badge
 * reads as the same object as the rows it summarizes.
...
function statusBadge(totals, compact) {
  ...
  if (compact) {
    ...
    return state === null ? null : h(StatusDot, { state: state });
  }
  var parts = [];
...
function MatrixDot(props) {
  return h("svg", {
    ...
    "aria-hidden": "true",
    style: { flexShrink: 0, display: "inline-block", color: props.color },
...
function StatusDot(props) {
  ...
  return spec.ongoing ? h(MatrixDot, { color: spec.color }) : h(Dot, { color: spec.color });
}
 ...
 *
 * They stay separate on purpose: the sidebar rule is scoped to the official
 * `data-slot` seam, while the dot keyframes have to be document-wide but are
 * namespaced by a prefixed class. Keeping them in separate tags lets the
 * sidebar rule's scoping be asserted on its own.
 * @returns a function removing both stylesheets.
 */
function installStyles() {
  var order = styleElement("sidebar-order", SIDEBAR_ORDER_CSS);
  var dot = styleElement("status-dot", STATUS_DOT_CSS);
  return function () { order.remove(); dot.remove(); };
}
...
module.exports.isUnreadPrompt = isUnreadPrompt;
module.exports.readPeer = readPeer;
module.exports.accountCompletions = accountCompletions;
module.exports.liveBaseline = liveBaseline;
module.exports.pendingCompletions = pendingCompletions;
