...
/**
 * The two font stacks the design system ships, named explicitly instead of
 * inherited.
 *
 * The app does set `--dsw-font-family` on `body`, so inheriting works TODAY —
 * but only by luck of where this plugin happens to render. Naming them makes
 * the choice deliberate and makes the split visible: prose rides the UI stack,
 * machine strings ride the code stack.
 *
 * The shipped stacks are repeated verbatim as the `var()` fallback because a
 * missing token invalidates the whole declaration at computed-value time —
 * `font-family` then becomes `unset` and the text silently inherits from
 * wherever it is, which is exactly the accident these lines exist to prevent.
 * The code stack deliberately has no bare `monospace` tail (Windows CJK would
 * otherwise fall back to SimSun), per `ui-theme/src/styles/base.css`.
 */
var FONT = {
  ui: "var(--dsw-font-family, -apple-system, BlinkMacSystemFont, 'Segoe UI', 'PingFang SC',"
    + " 'Hiragino Sans GB', 'Microsoft YaHei', 'Helvetica Neue', Helvetica, Arial, sans-serif)",
  code: "var(--ds-font-family-code, 'SF Mono', 'JetBrains Mono', 'Fira Code', Consolas,"
 ...
 * never clear. Comparing the age of something that HAPPENED against this
 * timestamp is what makes the green dot mean "activity since you last looked"
 * instead.
...
var PEER_STATUS_VERSIONS = [3, 4, 5];
 ...
 * non-peer: reachable, but with nothing to say. Version 3 predates `pending` and
 * version 4 predates `completedAgeMs`, so such a peer contributes less — never a
 * wrong count.
  ...
  return fetch(host.url + PEER_STATUS_PATH, {
    ...
    return response.json().then(function (body) {
      ...
      if (body.available !== true) {
        // The host is up but cannot say what its own sessions are doing: no
        // session controller, or its list read failed. Reporting zeros here
        // would make a broken peer look like an idle one — another "signal
        // silently lost". It lands in the unreachable bucket instead, whose
        // wording is already "state unreadable", and so shows the red dot.
        return Object.assign(base, { reachable: true, peer: false });
      }
      ...
      sessions.forEach(function (session) {
        ...
        if (isUnreadSession(session, elapsedMs)) unread += 1;
      });
...
/**
 * How long ago one peer session last did anything.
 *
 * Two durations describe a session, and the most recent event is the smaller of
 * them:
 *
 *  - `ageMs`, the age of its last HUMAN prompt. `SessionSummary.updatedAt` is
 *    `max(createdAt, lastPromptAt)`, so this is the only thing it can measure —
 *    and it does NOT move when a run ends, which is why a completion needs the
 *    second duration.
 *  - `completedAgeMs`, the age of the last finish the peer observed. Absent on a
 *    peer older than version 5, and absent when nothing has finished.
 *
 * Both are peer-measured durations, so the verdict never depends on the two
 * machines' clocks agreeing. Taking the smaller per ROW is what makes the count
 * a count of sessions: one session that was prompted and then finished yields
 * one value, not two.
 * @param session - one peer session row.
 * @returns milliseconds, or Infinity when the row carries no usable duration.
 */
function activityAgeMs(session) {
  var prompt = Number(session.ageMs);
  var finished = Number(session.completedAgeMs);
  var age = Number.isFinite(prompt) ? prompt : Infinity;
  if (Number.isFinite(finished) && finished < age) age = finished;
  return age;
}

/**
 * Whether one peer session did anything after this browser's last look.
 *
 * A running session is never "unread": it is reported by the running count
 * instead. A row with no usable duration is not evidence of recent activity, so
 * it counts as seen rather than as brand new.
 * @param session - one peer session row.
 * @param elapsedMs - milliseconds since this browser last viewed the host.
 * @returns true when the session's last event is newer than the last look.
 */
function isUnreadSession(session, elapsedMs) {
  if (session === null || typeof session !== "object") return false;
  if (session.running === true) return false;
  return activityAgeMs(session) < elapsedMs;
}
...
/**
 * One count chip: the state dot plus its count, on a tint of the state's own
 * color.
 *
 * The pairing copies the shipped `ConnectionIndicator` (a `*-tertiary` fill
 * under `*-primary` ink), except that the tint is MIXED from the primary rather
 * than read from a token: only success and warn define a tertiary step, and red
 * — the unreadable state — never got one. At 16% the mix lands within a few
 * levels of both designed steps, so one formula covers all four states.
 *
 * Type: the geometry is the shipped capsule spec, frozen in `Tag` ("a tag reads
 * as one size everywhere") — 11px/17px, weight 500, `1px 8px` padding, full
 * radius — not numbers of our own. The FAMILY is the UI stack, never the code
 * stack: the count is a quantity, and the design system types its quantities
 * that way (`StatsPills`/`TurnUsagePanel` say `font: inherit` and reach for
 * numerals instead; the code stack is reserved for identifiers and payloads,
 * which here are the host URL and the token). `tabular-nums` is what keeps a
 * `1` chip exactly as wide as an `8` chip, so the trailing cluster never
 * wobbles as the counts change.
 *
 * The color rides the chip, so the dot inside it, the number, and the fill are
 * one value. The chip is `flex: none`: a squeezed count is unreadable, and the
 * row has room to let the label ellipsize first.
 * @param state - one of the four states.
 * @param count - how many hosts are in it.
 * @param label - the tooltip words for that state.
 * @returns the chip element.
 */
function countBadge(state, count, label) {
  var color = DOT_STATES[state].color;
  return h("span", {
    key: label,
    "data-dsh-remote-count": state,
    title: label + " " + String(count),
    style: {
      display: "inline-flex", alignItems: "center", gap: 4, flex: "none",
      padding: "1px 8px", borderRadius: 999, cornerShape: "round",
      background: "color-mix(in srgb, " + color + " 16%, transparent)",
      color: color,
      fontFamily: FONT.ui, fontSize: 11, fontWeight: 500, lineHeight: "17px",
      fontVariantNumeric: "tabular-nums", whiteSpace: "nowrap",
    },
  }, h(StatusDot, { state: state }), h("span", null, String(count)));
}

/**
 * Render the session-state badge that rides the rail glyph.
 *
 * A WIDE row gets a trailing cluster of count chips — one per state, in the
 * same priority order the tooltip lists them — and the cluster is a flex item of
 * the row itself (see RemoteIcon), pinned with `margin-left: auto` to the
 * trailing edge. That seat is the one the session rows already give their time
 * stamp, and it is the whole point: the shell renders the row as
 * `[glyph][label]`, so a badge living INSIDE the glyph box lands in front of the
 * label and shoves it right — off the text column every other row shares.
 *
 * The COLLAPSED rail cell is a 36px square around an 18px glyph, with no label
 * and no room for one, so it carries a single dot in the highest-priority state
 * color, hung on the glyph's top-right corner the way a notification badge is,
 * and leaves the counts to the tooltip. Colors and the chase animation mirror
 * the sidebar's own session dots, so the badge reads as the same object as the
 * rows it summarizes.
 * @param totals - aggregate counts.
 * @param compact - true inside the 36px rail cell.
 * @param tooltip - the breakdown text when the caller has it. The badge carries
 *   its own disclosure because the structural wrapper cannot be relied on for
 *   one: in the wide row it dissolves into the flex row (`display: contents`),
 *   and a box-less element has no hover target of its own.
 * @returns the badge element, or null when there is nothing to say.
 */
function statusBadge(totals, compact, tooltip) {
  ...
  if (compact) {
    var state = waiting
      ? "waiting"
      : (running ? "running" : (unread ? "unread" : (unreachable ? "unreachable" : null)));
    if (state === null) return null;
    // Absolute, against the glyph box: anything in flow would sit BESIDE the
    // glyph and push it off the rail cell's center. The offsets are negative so
    // the dot hangs on the glyph's corner instead of inside its strokes, and the
    // ring is the sidebar's own fill so the dot reads as a notch, not a smudge.
    return h(StatusDot, {
      state: state,
      title: tooltip,
      style: {
        position: "absolute", top: -4, right: -4, borderRadius: "50%",
        boxShadow: "0 0 0 2px var(--dsw-specific-sidebar-fill, #1b1b1c)",
      },
    });
  }
  var parts = [];
  if (waiting) parts.push(countBadge("waiting", totals.waiting, "等待你回答"));
  if (running) parts.push(countBadge("running", totals.running, "运行中"));
  if (unread) parts.push(countBadge("unread", totals.unread, "上次查看后有活动"));
  if (unreachable) parts.push(countBadge("unreachable", totals.unreachable, "状态不可读"));
  if (parts.length === 0) return null;
  return h("span", {
    "data-dsh-remote-cluster": "",
    title: tooltip,
    style: { display: "inline-flex", alignItems: "center", gap: 6, flex: "none", marginLeft: "auto" },
  }, parts);
}
...
/**
 * The sidebar's own status dot, reproduced so the badge reads as the same
 * object as the remote's session rows rather than as a lookalike.
 *
 * `StateDot.module.css` draws a solid state as a 0.10-opacity halo at full size
 * with a solid core at 6/10 scale, both in `currentColor`; "ongoing" is instead
 * an eight-cell chase on a 10px grid, one cell per 2px step clockwise from the
 * top-left, each holding a discrete brightness step (flat keyframe holds, no
 * tweening — the retro feel), peaking when the chase reaches it and decaying
 * over the next three cells. The per-cell phase offset is the inline delay.
 */
var DOT_SIZE = 10;
...
/** Class the injected chase keyframes target. Prefixed: the rule is document-wide. */
var DOT_CELL_CLASS = "dsh-remote-dsh-cell";

/** One solid colored dot: halo plus core, in the given color. */
function Dot(props) {
  var layer = function (inset, extra) {
  ...
  return h("span", {
    ...
    style: Object.assign({
      ...
      width: DOT_SIZE, height: DOT_SIZE, color: props.color,
    }, props.style),
  }, h("span", { style: layer(0, { opacity: 0.1 }) }), h("span", { style: layer("20%", {}) }));
}

/** The animated "ongoing" dot: eight cells chasing clockwise, forever. */
function MatrixDot(props) {
  return h("svg", {
    ...
    "aria-hidden": "true",
    title: props.title,
    style: Object.assign({ flexShrink: 0, display: "inline-block", color: props.color }, props.style),
  }, MATRIX_CELLS.map(function (cell, index) {
    return h("rect", {
      key: String(cell[0]) + "x" + String(cell[1]),
      x: cell[0],
      y: cell[1],
      width: 2,
      height: 2,
      className: DOT_CELL_CLASS,
      // Negative delays phase the chase so every cell animates from mount.
      style: { animationDelay: String((index - MATRIX_CELLS.length) * 125) + "ms" },
    });
  }));
}

/** The four badge states, each with its color and whether it animates. */
var DOT_STATES = {
  waiting: { color: COLOR.attention, ongoing: false },
  running: { color: COLOR.running, ongoing: true },
  unread: { color: COLOR.done, ongoing: false },
  unreachable: { color: COLOR.failure, ongoing: false },
};

/**
 * One badge dot: the chase while anything is running, a solid dot otherwise.
 *
 * Placement and disclosure ride through from the caller: the two shapes are
 * interchangeable in every state, and only the caller knows whether this dot
 * leads a count chip or hangs on the rail glyph's corner.
 * @param props - state, plus optional inline style and tooltip.
 * @returns the dot element.
 */
function StatusDot(props) {
  var spec = DOT_STATES[props.state];
  var shape = { color: spec.color, style: props.style, title: props.title };
  return spec.ongoing ? h(MatrixDot, shape) : h(Dot, shape);
}
...
/**
 * The rail glyph: `ic_ds_globe_outline_14`, the shipped globe from
 * `@deepseek-ai/dsh-client-ui-primitives`, inlined as path data.
 *
 * Inlined, not required, because this bundle is deliberately dependency-free:
 * the client module table answers a bare specifier only when that package has
 * already materialized in the boot graph, and a miss THROWS — inside `apply`,
 * which would take the whole panel down with it. Copied geometry cannot fail to
 * load. It also replaced a hand-drawn stroked globe that read visibly lighter
 * than the shell's own filled 16px glyphs sitting directly above it.
 */
var GLOBE_PATH = "M7.00018 0.353516C10.6708 0.353535 13.6468 3.32958 13.6469 7.00018C13.6468 10.6708"
  + " 10.6708 13.6468 7.00018 13.6469C3.32957 13.6468 0.353535 10.6708 0.353516 7.00018C0.353535 3.32957"
  + " 3.32957 0.353531 7.00018 0.353516ZM5.44643 7.59661C5.49463 8.97506 5.70762 10.191 6.02136 11.0793C6.20141"
  + " 11.5891 6.40328 11.9585 6.59898 12.1889C6.79501 12.4196 6.93213 12.454 7.00018 12.454C7.06822 12.454"
  + " 7.20533 12.4197 7.40138 12.1889C7.59708 11.9585 7.79895 11.589 7.979 11.0793C8.29274 10.191 8.50574"
  + " 8.97506 8.55394 7.59661H5.44643ZM1.57861 7.59661C1.80785 9.70467 3.2386 11.4509 5.1715 12.1388C5.07135"
  + " 11.9317 4.97972 11.7098 4.89746 11.477C4.53084 10.4391 4.30224 9.0828 4.25357 7.59661H1.57861ZM9.74679"
  + " 7.59661C9.69813 9.0828 9.46952 10.4391 9.1029 11.477C9.0206 11.7099 8.92818 11.9316 8.82797 12.1388C10.7613"
  + " 11.4511 12.1925 9.70496 12.4218 7.59661H9.74679ZM5.1706 1.8616C3.23814 2.54963 1.80876 4.29604 1.5795"
  + " 6.40376H4.25357C4.30224 4.91756 4.53083 3.56129 4.89746 2.5234C4.97968 2.29066 5.07051 2.0686 5.1706"
  + " 1.8616ZM7.00018 1.54637C6.93213 1.54638 6.79503 1.5807 6.59898 1.81145C6.40332 2.04177 6.20139 2.41058"
  + " 6.02136 2.92012C5.70754 3.80851 5.49461 5.02499 5.44643 6.40376H8.55394C8.50575 5.025 8.29282 3.80851"
  + " 7.979 2.92012C7.79898 2.41059 7.59705 2.04177 7.40138 1.81145C7.20531 1.58067 7.06823 1.54637 7.00018"
  + " 1.54637ZM8.82887 1.8616C8.92902 2.0687 9.02064 2.29053 9.1029 2.5234C9.46953 3.56129 9.69812 4.91756"
  + " 9.74679 6.40376H12.4209C12.1916 4.29575 10.7618 2.54943 8.82887 1.8616Z";

/**
 * The rail icon. Icon only: the shell supplies the button, the accessible name
 * from the list registration's label, and the label element itself, and wraps
 * this in an `aria-hidden` glyph span.
 *
 * In the wide row the returned wrapper DISSOLVES (`display: contents`), which is
 * load-bearing rather than cosmetic. The shell renders the row as
 * `[glyph span][label span]` and the badge lives in here, so with the wrapper in
 * flow the cluster would be laid out in front of the label — the layout this
 * badge is fixing. Dissolved, the glyph and the cluster become flex items of the
 * row around the shell's label, and the cluster's `margin-left: auto` reaches the
 * row's trailing edge. `SIDEBAR_SEAM_CSS` dissolves the shell's own glyph span
 * for the same row, so the chain has no box left in it.
 *
 * The rail cell keeps the wrapper as a real 18px box: that box is what the
 * corner dot is positioned against.
 * @param props - framework props; `size` is 16 in the wide row and 18 in the rail.
 * @returns the glyph, with the badge when there is one.
 */
function RemoteIcon(props) {
  ...
  var tooltip = statusTooltip(snapshot.totals, snapshot.hosts);
  var badge = statusBadge(snapshot.totals, compact, tooltip);
  var glyph = h("svg", {
    viewBox: "0 0 14 14", width: size, height: size, fill: "none",
    "aria-hidden": "true",
    // The row's own marker, and the reason it is HERE rather than on the badge
    // wrapper: the seam rules that dissolve the glyph box and type the label
    // must match the row in every state, and `data-dsh-remote-badge` only
    // exists while there is something to report. The glyph is the one element
    // this plugin always draws. It is inert otherwise — no style hangs on the
    // attribute itself, and the `<svg>` stays the same box either way.
    "data-dsh-remote-glyph": "",
  }, h("path", { fillRule: "evenodd", clipRule: "evenodd", fill: "currentColor", d: GLOBE_PATH }));
  if (badge === null) return glyph;
  return h("span", {
    "data-dsh-remote-badge": compact ? "rail" : "row",
    title: tooltip,
    style: compact
...
function RemoteWorkspace(props) {
  ...
  var bar = h("div", {
    ...
    showSettings && selected !== null && form === null
      ? h("div", {
          ...
          // A URL is a machine string: the code stack is what keeps every
          // character of it distinguishable. `<code>` would NOT do that on its
          // own — the UA sheet pins it to a bare `monospace`, the one stack
          // ui-theme's base.css documents as unsafe for a CJK host.
          h("code", { style: { color: COLOR.faint, fontFamily: FONT.code } }, selected.url),
          h("span", { style: { flex: 1 } }),
          h("input", {
            value: token,
            ...
            style: {
              // `font: inherit` first (it resets the UA's pinned control font),
              // then the two overrides that make it a machine string.
              font: "inherit", fontFamily: FONT.code, fontSize: 12,
              width: 260, padding: "4px 8px", borderRadius: 6,
  ...
  if (form !== null) {
    body = h("form", {
      ...
      h("input", {
        ...
        style: {
          // The address is a machine string too, and one the operator has to
          // read character by character before saving it.
          font: "inherit", fontFamily: FONT.code,
  ...
  return h("div", {
    ...
    style: {
      ...
      // One place names the family for everything this plugin draws: the bar,
      // the settings row, the notices and every control inside them inherit it,
      // and the two machine strings below override it with the code stack.
      fontFamily: FONT.ui,
    },
  },
    bar,
...
/**
 * The row label's type — the one piece of text this plugin does not draw.
 *
 * The shell renders `[glyph span][label span]` and owns the label, so this rule
 * styles an element the plugin did not create. The sidebar has no label slot, so
 * the seat's default type is body copy rather than an entry name, and that is
 * what the label reads as today.
 *
 *  - FAMILY: the UI stack, named rather than inherited — the sidebar root sets
 *    no family at all, so the label resolves against whichever ancestor happens
 *    to carry `--dsw-font-family`. That is the same accident the takeover
 *    surface stopped relying on, and on a CJK label the stack's first
 *    CJK-capable family is what actually decides the glyphs.
 *  - SIZE: 14px, the sidebar's own row-label size: session rows are 14/20
 *    (`ui-workspace` `Rows.module.css` `.title`), the shell's `.panelRow`
 *    already inherits 14px from `.root`, and New Session is 14/500. Dropping a
 *    navigation entry to 13px would read as a caption.
 *  - WEIGHT: 500. The row's ink is `--dsw-alias-label-secondary`, one step below
 *    the session rows' primary, so weight is what keeps a dimmed entry from
 *    reading as disabled. It is the weight this seat's siblings use (New Session
 *    500, brand 600) and the shell's own active-row weight, so selecting the row
 *    then changes its ink and fill only, not its weight.
 *  - NOTHING ELSE: no tracking (the system reserves letter-spacing for brand and
 *    eyebrow text — 0.04em on the 18px brand — and this is neither), no
 *    line-height (the row is `min-height: 36px` and centres its items, so one
 *    here would be invisible), and no ink override (hover and active ink belong
 *    to the shell).
 */
var LABEL_TYPE_CSS = 'font-family:' + FONT.ui + ';font-size:14px;font-weight:500';

/**
 * Raise this plugin's rail row above the local "New Session" button, and give
 * the count cluster somewhere to land.
 *
 ...
 *
 * The third rule dissolves the shell's glyph span for THIS row only, keyed on
 * our own marker. The shell renders a panel row as `[glyph span][label span]`
 * and wraps the registered icon in that glyph, so as long as it has a box the
 * count chip is stuck to the glyph and lands in front of the label, shoving the
 * label off the text column every other row shares. Dissolved, the glyph and the
 * chip become flex items of the row around the shell's label, which is what lets
 * the chip sit at the trailing edge. `:has()` keeps every other panel's glyph
 * untouched, and `RemoteIcon` dissolves its own wrapper to match.
 *
 * The row anchor is the GLYPH marker, not the badge marker. Dissolving the glyph
 * box and hanging the label's type on the row must hold whether or not a badge
 * is present, and `data-dsh-remote-badge` only exists while there is something to
 * say. The glyph is the one element this plugin always draws, so it is the
 * honest anchor. `RemoteIcon` puts the marker on the `<svg>` itself.
 *
 * The `order: 1` rule is what puts the label back on the text column. Dissolving
 * the glyph makes the cluster a flex item of the row, but it stays BEFORE the
 * shell's label in source order, and its `margin-left: auto` then eats the free
 * space — so the chip still ends up left of the label and the label is pushed to
 * the trailing edge (measured: label at 188px in a 248px row instead of 32px).
 * Moving the cluster after the label restores `[glyph][label] ······ [chips]`:
 * the label keeps the shell's own 8px rhythm, and the cluster lands on the
 * trailing edge — 8px in, the seat the session rows give their time stamp.
 *
 * The next rule is insurance, not layout: it puts a containing block on this row
 * so the rail's corner dot can never resolve against the initial containing
 * block and land at the top-right of the PAGE. The dot is positioned against the
 * icon wrapper, which is a box in the rail and only because `RemoteIcon` reads
 * the shell's 16/18 size prop to know which mode it is in; if that prop ever
 * stopped distinguishing the two, a dot that lost its wrapper would still be
 * clamped to this row. `position: relative` on a row with no other positioned
 * descendant paints and measures exactly the same.
 *
 * The last rule types the row's label, which the SHELL owns and renders — this
 * seat has no label slot. It is the only thing here that styles text the plugin
 * did not create, so it is deliberately the smallest possible intervention:
 * family, size, weight, nothing else. See `LABEL_TYPE_*` for why those three.
 *
 ...
 * hashing (`WNUpnq_panelList`). If the shell renames a class the rule simply
 * stops matching and the row keeps today's layout — the failure mode is "no
 * change", never breakage.
 *
 * The order rules move the whole panellist nav, not one row: every global panel
 ...
 */
var SIDEBAR_SEAM_CSS = '[data-slot="sidebar"] [class*="logoRow"]{order:-2}'
  + '[data-slot="sidebar"] [class*="panelList"]{order:-1}'
  + '[data-slot="sidebar"] [class*="panelGlyph"]:has([data-dsh-remote-glyph]){display:contents}'
  + '[data-slot="sidebar"] [class*="panelRow"]:has([data-dsh-remote-glyph]){position:relative}'
  + '[data-slot="sidebar"] [class*="panelRow"]:has([data-dsh-remote-glyph])'
  + ' [data-dsh-remote-cluster]{order:1}'
  + '[data-slot="sidebar"] [class*="panelRow"]:has([data-dsh-remote-glyph])'
  + ' [class*="panelTitle"]{' + LABEL_TYPE_CSS + '}';

/**
 * The chase keyframes for the running dot, copied verbatim from the sidebar's
 * own `StateDot.module.css`.
 *
 * They have to be injected rather than expressed inline: React inline styles
 * cannot carry `@keyframes`, and the class is what the rects animate through.
 * The opacity steps are flat holds a tenth of a percent apart, which is what
 * makes each cell snap instead of fade.
 */
var STATUS_DOT_CSS = '.' + DOT_CELL_CLASS + '{fill:currentColor;opacity:.15;'
  + 'animation:dsh-remote-dsh-chase 1s infinite}'
  + '@keyframes dsh-remote-dsh-chase{'
  + '0%,12.4%{opacity:1}12.5%,24.9%{opacity:.6}25%,37.4%{opacity:.35}37.5%,100%{opacity:.15}}';

/** One tagged style element, appended to head. */
function styleElement(tag, css) {
  var style = document.createElement("style");
  style.setAttribute("data-dsh-remote-dsh", tag);
...
/**
 * Install the two stylesheets this plugin needs and return their disposer.
 *
 * They stay separate on purpose: the seam rules are scoped to the official
 * `data-slot` anchor, while the dot keyframes have to be document-wide but are
 * namespaced by a prefixed class. Keeping them in separate tags lets the seam
 * rule's scoping be asserted on its own.
 * @returns a function removing both stylesheets.
 */
function installStyles() {
  var seam = styleElement("sidebar-seam", SIDEBAR_SEAM_CSS);
  var dot = styleElement("status-dot", STATUS_DOT_CSS);
  return function () { seam.remove(); dot.remove(); };
}
...
function apply(ctx) {
  ...
  ctx.effect(function () { return installStyles(); }, "dsh-remote-dsh: styles");
...
module.exports.isUnreadSession = isUnreadSession;
module.exports.activityAgeMs = activityAgeMs;
module.exports.readPeer = readPeer;
