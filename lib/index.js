 ...
 * 5 — a row may additionally carry `completedAgeMs`.
 *
...
/**
 * What a peer has to remember between two reads.
 *
 * Two facts cannot be read off a single `sessionController.list()` snapshot, and
 * both are needed to report this host's state honestly:
 *
 *  - **A pending interaction.** The browser assembles the amber sidebar dot from
 *    pending-interaction domains that `ui-approval` and `ui-user-questions`
 *    register, so no Host service exposes the state as data. What the Host DOES
 *    have is the request itself: both packages dispatch a scope-filtered
 *    waterfall and only settle when an answerer returns. Holding those two
 *    waterfalls open is therefore an exact reading of "this session is waiting
 *    for a human". A session may hold more than one open request (a nested ask),
 *    so each entry is keyed by an opaque token and the reported kind is the
 *    highest-ranked one still open — the same precedence the client's pending
 *    domains use.
 *
 *  - **A finished run.** `updatedAt` is the age of the last HUMAN prompt
 *    (`max(createdAt, lastPromptAt)`), which does not move when a run ends, so
 *    "finished a second ago" and "idle, never ran" are the same snapshot. Only
 *    the running→idle EDGE says it — and an edge needs a previous observation.
 *    It is kept HERE rather than in the reader because the register is keyed by
 *    the real session id, which lets each completion land on the session that
 *    produced it. A count of edges held by the reader instead cannot tell "one
 *    session finished twice" from "two sessions finished", nor separate a
 *    session that was merely prompted from one that actually ran.
 *
 * Neither map is ever serialized: the id is used only as an internal key, and
 * rows leave this process anonymous.
 *
 * @returns the register: `track`/`kindOf`/`entries` for pending requests,
 *   `observeRunning`/`completedAgeMs` for finishes, and `retain` to forget
...
export function createPeerTracker() {
  ...
  return {
    ...
    kindOf,
    /**
     * Note one session's liveness and stamp a finish on the falling edge.
     *
     * `live` counts a session that is running as live EVEN IF it is blocked on
     * the operator: moving from busy to waiting is not a finish, and that state
     * already lights the amber dot. A session observed for the first time only
     * records its bit — sessions that are already idle at load get no reminder,
     * which is also what the remote's own sidebar does.
     * @param sessionId - the session just listed.
     * @param live - whether it is running right now.
     * @param now - the read's clock.
     */
    observeRunning(sessionId, live, now) {
      const previous = prevRunning.get(sessionId)
      prevRunning.set(sessionId, live)
      if (live) {
        // Running again supersedes the reminder, exactly as it does in the
        // browser-side session manager.
        completedAt.delete(sessionId)
        return
      }
      if (previous === true) completedAt.set(sessionId, now)
    },
    /**
     * How long ago one session finished a run.
     * @param sessionId - session to read.
     * @param now - the read's clock.
     * @returns milliseconds since the finish, or undefined when none is pending.
     */
    completedAgeMs(sessionId, now) {
      const at = completedAt.get(sessionId)
      return at === undefined ? undefined : Math.max(0, now - at)
    },
    /**
     * Forget sessions that are gone.
     *
     * The two sets differ on purpose, and swapping them loses a real signal:
     *
     *  - Liveness is only ever observed for the rows this read REPORTS, so its
     *    two maps must be pruned to exactly that set — otherwise a session that
     *    stopped being reported keeps a baseline it can never fall from.
     *  - A request can be raised by ANY listed session, a subagent included (it
     *    is then attributed to its root). Pruning those to the reported set would
     *    delete a blocked subagent's request on the very read that reported it,
     *    so the amber dot would survive one poll and then vanish while the
     *    subagent was still blocked.
     * @param sets - `reported` ids (rows emitted) and `listed` ids (every
     *   session the controller returned, subagents and blanks included).
     */
    retain({ reported, listed }) {
      const keepReported = new Set(reported)
      for (const key of Array.from(prevRunning.keys())) {
        if (!keepReported.has(key)) prevRunning.delete(key)
      }
      for (const key of Array.from(completedAt.keys())) {
        if (!keepReported.has(key)) completedAt.delete(key)
      }
      const keepListed = new Set(listed)
      for (const key of Array.from(bySession.keys())) {
        if (!keepListed.has(key)) bySession.delete(key)
      }
    },
    /**
...
/**
 * Registration options that put the observer AHEAD of every answerer.
 *
 * This is load-bearing, not a preference. Cordis runs waterfall listeners
 * outermost-first and a listener that does not call `next()` vetoes the rest of
 * the chain. The Remote Events bridge (`api-remotes`) is exactly such a
 * listener: it hands the request to the browser and holds the chain open until
 * the human answers, and it registers EAGERLY when the gateway installs the
 * forwarded-event source — at boot, before any patch-layer plugin mounts. An
 * observer registered after it is therefore never called while a request is
 * pending, which is the only moment its state matters: the session shows amber
 * on the remote and nothing at all here.
 *
 * Measured against the real framework (real `Context`, real `scopeTarget`
 * carrier, a bridge-shaped listener that waits): registered after the bridge the
 * tracker stays empty; registered with `prepend` the row reports
 * `pending: 'question'`. Being first costs nothing — {@link holdPending} always
 * delegates with `next()`, so the request still reaches the browser that answers
 * it, and the tracker is released when that chain settles.
 */
const OBSERVE_FIRST = Object.freeze({ prepend: true });
 ...
 * reaches the browser that will answer it. The one thing the registration does
 * insist on is its POSITION in the chain — see {@link OBSERVE_FIRST}.
 ...
 *
 * The payload is deliberately anonymous — `{ running, ageMs, pending?,
 * completedAgeMs? }` rows with no session ids, titles, or content — because the
 * reader is another machine's browser, and counts are all the badge needs.
 *
 * Why durations (`ageMs`, `completedAgeMs`) instead of absolute timestamps: the
 * reader compares "how long ago this happened" against "how long ago I last
 ...
 *
 * `ageMs` is the age of the last HUMAN prompt and `completedAgeMs` the age of
 * the last observed finish, so a reader takes the smaller of the two to get how
 * long ago the session last did anything. Both ride the same row, which is what
 * keeps the reader's count a count of SESSIONS: two signals on one session
 * collapse into one, and two sessions stay two.
 *
 ...
 * @param ctx - host context.
 * @param tracker - the peer's live register, or undefined for a bare read.
...
export async function collectSelfStatus(ctx, tracker) {
  ...
  for (const item of items) {
    if (item === null || typeof item !== 'object') continue
    // Every session the controller returned, before any row filtering: a request
    // can be raised by a session that is not a row of its own.
    if (typeof item.sessionId === 'string') listedIds.push(item.sessionId)
    ...
    const pending = sessionId === undefined ? undefined : pendingOf.get(sessionId)
    // Liveness is observed only for the rows this read reports, so a subagent's
    // own runs do not surface as a completion on its parent.
    let completedAgeMs
    if (sessionId !== undefined && tracker !== undefined && typeof tracker.observeRunning === 'function') {
      tracker.observeRunning(sessionId, running, now)
      completedAgeMs = tracker.completedAgeMs(sessionId, now)
      reportedIds.push(sessionId)
    }
    sessions.push({
      running,
      ageMs: updatedAt === 0 ? Number.MAX_SAFE_INTEGER : Math.max(0, now - updatedAt),
      ...(pending === undefined ? {} : { pending }),
      ...(completedAgeMs === undefined ? {} : { completedAgeMs }),
    })
    if (sessions.length >= STATUS_SESSION_CAP) break
  }
  if (tracker !== undefined && typeof tracker.retain === 'function') {
    tracker.retain({ reported: reportedIds, listed: listedIds })
  }
 ...
 * @param ctx - host context.
 * @param tracker - the peer's live register, or undefined.
 * @returns one prefix route.
 */
...
export function apply(ctx, config) {
  ...
  if (role === 'peer') {
    const pending = createPeerTracker()
    ctx.effect(
