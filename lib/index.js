...
export function createPeerTracker() {
  ...
  return {
    ...
    /**
     * Forget every session outside the given set.
     * @param listedIds - the session ids this read produced rows for.
     */
    retain(listedIds) {
      const keep = new Set(listedIds)
      for (const key of Array.from(prevRunning.keys())) {
        if (!keep.has(key)) prevRunning.delete(key)
      }
      for (const key of Array.from(completedAt.keys())) {
        if (!keep.has(key)) completedAt.delete(key)
      }
      for (const key of Array.from(bySession.keys())) {
        if (!keep.has(key)) bySession.delete(key)
      }
    },
    /**
...
export async function collectSelfStatus(ctx, tracker) {
  ...
  if (tracker !== undefined && typeof tracker.retain === 'function') tracker.retain(listedIds)
