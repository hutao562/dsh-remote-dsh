...
**为什么在最右端,而不是紧跟图标**:外壳把面板行渲染成 `[图标槽][文字]`,`renderSlot` 的内容住在图标槽里 —— 徽标只要还在文档流里,就排在文字**前面**,把「远程」两个字顶到右边、和别的行对不齐(第一版就是这样,用户一眼就看出来了)。`SIDEBAR_SEAM_CSS` 因此用 `:has([data-dsh-remote-glyph])` **只对本行**把外壳的图标槽 `display:contents` 掉,图标和胶囊才成为行本身的 flex item。那一列不是随手取的:会话行的时间戳同样距行右缘 8px,两者在同一条竖线上。

> **`margin-left:auto` 一个人不够。** 图标槽溶解之后,胶囊确实成了行的 flex item,但它仍然排在**外壳的文字之前**,于是 `auto` 外边距吃掉的空档落在胶囊左边,被顶到最右的其实是**文字** —— 胶囊反而贴在文字左侧,也就是用户最早报的那个样子。真正把顺序扳回来的是 `order:1`(胶囊排到文字之后)。**用真实样式表量过**:248px 的行里,文字原本落在 188px(文字列应该从 32px 开始),胶囊距行右缘 44px(应该 8px);加了 `order:1` 之后是 32px / 8px。这条几何断言进不了 jsdom 冒烟测试(jsdom 不做布局),所以它靠一张用真实样式表、真实浏览器量出来的排版预览图核对,不在自动测试里。标记放在**图标**(`data-dsh-remote-glyph`)而不是徽标上,是因为「溶解图标槽」和「给文字定字体」这两件事在**没有徽标时也必须成立**。
